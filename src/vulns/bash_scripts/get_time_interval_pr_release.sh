...
cd ../../../cloned_packages
echo "Vuln ID;Severity;Package name;Clone dir;Commit hash;Release;Release type;Commit date;Release date;Time interval" > $VULNS_OUTPUT
echo "Package name;Clone dir;Tot vulns with commit;# Severity L;# Severity M;# Severity H;Median severity;# Major v.;# Minor v.;# Patch v.;Median release type;Avg time interval" > $PACKAGE_OUTPUT

while IFS=';' read -r id severity package_name clone_dir commit_url commit_hash 
do
    if [ $i -ge $START ]
    then
        if [ $i -eq $START ]
        ...
        then
            if [ ! -d "../$clone_dir" ]; then echo "'$clone_dir' doeans't exists." >> $ERRORS_LOG
                ...
                then 
                    ...
                    echo "$current_package;$current_dir;$vulns;$severity_L;$severity_M;$severity_H;$median_pkg_severity;$major_v;$minor_v;$patch_v;$median_pkg_release_type;$avg_package_time_interval" >> ../$PACKAGE_OUTPUT
        ...
        if [[ $releases_string == *"error:"* ]]; then echo "ID '$id' doesn't belong to '$current_package'." >> $ERRORS_LOG
            ...
            if [[ $commit_date_command == *"fatal:"* ]]; then echo "ID '$id' doesn't have a commit date." >> $ERRORS_LOG
                ...
                if [ -z "$commit_date" ]; then echo "ID '$id' doesn't have a commit date." >> $ERRORS_LOG
                    ...
                    if [ $tags -gt 0 ]
                    then
                        echo "$id;$severity;$package_name;$clone_dir;$commit_hash;$tag;$release_type;$commit_date;$release_date;$time_interval" >> ../$VULNS_OUTPUT
